...
def prepare_and_train_model(data):
    features = data[['Consumption']].values
    target = data['Consumption'].shift(-1).fillna(method='ffill').values
...
def predict_consumption(model, X_test):
...
def display_consumption_data(df, site, year, unit):
    ...
    st.write("Les données de consommation sont générées de manière simulée pour le site sélectionné. Dans le futur, nous prévoyons d'intégrer les données réelles des entreprises partenaires en utilisant les API de RTE pour obtenir des données de consommation en direct.")

    if site == "Site A":
        filtered_data = df[df['Timestamp'].dt.year == year]
        total_consumption = filtered_data['Consumption'].sum() / 1000
        if unit == "MWh/jour":
            consumption_stats = filtered_data.set_index('Timestamp').resample('D').sum() / 1000
        else:
            consumption_stats = filtered_data.groupby(filtered_data['Timestamp'].dt.floor('H')).mean() / 1000
        fig = px.area(consumption_stats, consumption_stats.index, y='Consumption',
        ...
        display_consumption_stats(df, [2020, 2021, 2022, 2023])
        # rajout de l'appel à display_consumption_stats ici
